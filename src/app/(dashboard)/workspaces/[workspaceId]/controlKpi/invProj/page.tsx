...
const generateWeeklyData = (sku: string, region: string, scenario: string): InventoryData[] => {
  ...
  return [
    ...
    {
      parameter: "Projected Stock / Days Supply",
      unit: "units ('000) / days",
      weeks: forecast.map((val, idx) => {
        const stock = Math.round(val * (1.47 + idx * 0.08))
        const days = [40, 42, 45, 48, 50, 52, 55, 58, 60, 62, 65, 67][idx]
        const adjustedDays = scenario === "optimistic" ? Math.round(days * 0.9) : scenario === "pessimistic" ? Math.round(days * 1.2) : days
        return `${stock} / ${adjustedDays}`
      }),
      colorCode: true,
    },
    {
...
const getCellColor = (parameter: string, value: number | string): string => {
  ...
  if (parameter === "Projected Stock / Days Supply") {
    // For combined values, extract days and use days supply color
    if (typeof value === "string" && value.includes("/")) {
      const days = parseInt(value.split("/")[1].trim())
      return getDaysSupplyColor(days)
    }
    return ""
  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <ul className="text-sm space-y-1 ml-4">
                ...
                <li>
                  <span className="inline-block w-4 h-4 bg-yellow-400 mr-2"></span>Yellow: Stock &lt; 20K units (Below minimum threshold)
                </li>
                <li>
                  <span className="inline-block w-4 h-4 bg-green-500 mr-2"></span>Green: 20K-80K units (Optimal inventory levels)
                </li>
                <li>
                  <span className="inline-block w-4 h-4 bg-blue-500 mr-2"></span>Blue: Stock &gt; 80K units (Overstock situation)
                </li>
              </ul>
            </div>
